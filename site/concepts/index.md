...
my-mod/
  ...
  assets/
    replacements/       # swap vanilla assets by name
      ...
      models/
      audio/
    additions/          # new data assets a clone references
      sprites/
      textures/
      audio/
  templates/            # KDL: patches and clones
    weapon-foo.kdl
    skill-bar.kdl
  code/                 # C# SDK mod, dormant until used
  unity/                # Unity project for prefabs and UXML, dormant until used
  compiled/             # build output (don't edit, don't commit)
    jiangyu.json
    bundles/            # compiled asset bundles
      <mod-name>.bundle
    code/               # compiled mod DLL, when code/ is used
```

`jiangyu init` scaffolds `code/` (the [SDK](/sdk/)) and `unity/` (the [Unity project](/unity-project)) too, both dormant until you add code or assets. Everything except `compiled/` is author-owned. `compiled/` is regenerated by `jiangyu compile`.
