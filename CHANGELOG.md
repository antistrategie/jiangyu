...
- (Loader) Loads a mod's bundled sprites, textures, audio clips and prefabs on first use instead of all at start-up
- (Compiler) Compiled addition textures to DXT5 and addition sprites to BC7 wherever their dimensions divide by four
- (Compiler) Compiled audio clips to Vorbis held compressed in memory, in place of PCM decoded at load. Clips above 48 kHz keep PCM
- (Studio) Added the visual editor's subtype picker to every polymorphic list
