...
- Passes run on the scene-load poll and again when a UI screen is activated. A screen's own textures only enter the object graph when it is built, which is later than the scene-load pass, so a poll-only replacement lands several frames after the screen has already painted the original.
