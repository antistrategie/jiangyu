...
- At runtime, the loader finds every loaded game `Texture2D` whose `name` equals `<target-name>` and mutates its pixel data in place. The mutation path:
  0. Stamp `HideFlags.DontUnloadUnusedAsset` on the destination. Without it a texture whose pixels stream from a `.resS` side file can be unloaded and re-read, silently restoring the vanilla image; the pin is what makes the mutation survive a scene change, and it keeps that one texture resident for the session.
  1. `Graphics.Blit` the replacement into an sRGB `ARGB32` `RenderTexture` sized to the destination. No mip settings on that RenderTexture: `GetTemporary` returns an already-created texture, and only level 0 is written and read.
  2. `ReadPixels` into a readable staging `Texture2D` whose format matches the destination's alpha-presence (`RGB24` for `DXT1`/`BC1` no-alpha targets, `RGBA32` for `DXT5`/`BC3` with-alpha targets) and whose **mip count is the destination's**, not a full chain. Alpha-matched staging is what steers Unity's managed compressor to produce the right compressed family on the next step; the matched mip count is what lets step 4 copy at all.
  ...
  4. `Graphics.CopyTexture` from staging into the game texture. The copy is a straight memory copy and does not resample, so it needs matching size, format and mip count; staging is built to match on size and mips, leaving the format as the one live condition. Fall back to `Graphics.ConvertTexture` only for compressed formats that managed `Compress` can't produce (BC7, ASTC, BC4/5/6H).
...
- The mutation is idempotent per `Texture2D` instance per scene: once mutated, an instance is skipped on later apply passes, and the dedupe state is dropped on scene unload because the instance ids belong to objects the scene owned.
- Passes run on the scene-load poll and again when a UI screen is activated. A screen's own textures only enter the object graph when it is built, which is later than the scene-load pass, so a poll-only replacement lands several frames after the screen has already painted the original. Each screen instance gets at most two activation-time sweeps: it retires once a sweep places something or the budget is spent, and a texture that enters the graph later is placed by the poll.
