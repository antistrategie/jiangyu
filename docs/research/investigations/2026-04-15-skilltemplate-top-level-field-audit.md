...
Classify the 120 vs 128 field delta between Jiangyu's observed `SkillTemplate` serialised contract and the legacy schema at `../MenaceAssetPacker/generated/schema.json`.

## Why This Target

The SkillOnSurfaceDefinition spot-check (same date) found all 3 inspected `SkillTemplate` samples showing 120 serialised fields under `m_Structure`, while the legacy schema lists 128. The 8-field net delta was flagged as a candidate for a top-level field audit. With the first `SkillTemplate` nested type now validated, the top-level audit is the natural next step.
...
All 3 samples show **120 serialised fields** under `m_Structure` with an identical field set (names, kinds, type names). The field set is stable across all 3 skill categories.
...
These are the same 4 base class fields that appeared as legacy-only in EntityTemplate (where `m_LocalizedStrings` was also absent) and WeaponTemplate (where all 4 plus `ItemType` were absent). The explanation is consistent: Unity's serialisation rules exclude them from the current serialised contract.

#### Group 2: Odin-serialised fields (5 fields)

This is a **new classification** not seen in prior passes. These 5 fields have interface or abstract types that Unity's native serialisation cannot handle.

| Field | Legacy type | Legacy category | Reason absent from Jiangyu |
|---|---|---|---|
| `CustomAoEShape` | ICustomAoEShape | reference | **interface** type — excluded by Unity serialisation |
| `AoEFilter` | ITacticalCondition | reference | **interface** type — excluded by Unity serialisation |
| `ProjectileData` | BaseProjectileData | reference | **abstract** base class — excluded by Unity serialisation |
| `SecondaryProjectileData` | BaseProjectileData | reference | **abstract** base class — excluded by Unity serialisation |
| `AIConfig` | SkillBehavior | reference | likely Odin-routed — not confirmed as abstract/interface directly, but absent from Unity's serialised type tree and consistent with the Odin pattern |

AssetRipper's `FieldSerializer.Logic.cs` explicitly checks `!resolvedTypeDeclaration.IsAbstract` and `!resolvedTypeDeclaration.IsInterface` before including fields in the serialisable type. Fields that fail these checks are excluded from the type tree entirely.
...
- combat skills with projectiles have more Odin-serialised data and Unity object references (tracers, grenades)
...
This is circumstantial but strongly consistent. The actual data is present in the serialised asset — it is just stored in the Odin blob, not as separate Unity-native fields.
...
This is the Odin Serializer container. It is a concrete public field that passes Unity's serialisation filters, so AssetRipper includes it in the type tree. The legacy schema does not list it because it is a serialisation framework field, not a SkillTemplate-specific gameplay field.
...
- the 120-field serialised contract is stable across 3 skill categories (active combat, passive, active utility)
...
- the 5 Odin-routed legacy-only fields are a new classification: interface/abstract-typed fields that Unity cannot natively serialise, routed through Odin Serializer into the `serializationData` blob
...
| legacy broader than serialised contract (base class / managed-only) | 4 | `m_ID`, `m_IsGarbage`, `m_IsInitialized`, `m_LocalizedStrings` |
| legacy broader than serialised contract (Odin-routed) | 5 | `CustomAoEShape`, `AoEFilter`, `ProjectileData`, `SecondaryProjectileData`, `AIConfig` |
| Jiangyu-only (serialisation framework) | 1 | `serializationData` |
| real mismatch | 0 | — |

The new finding is that `SkillTemplate` is the first template type where the Jiangyu-vs-legacy delta includes Odin-serialised fields, not just base class exclusions. This means the legacy schema includes fields from both Unity's native serialisation and Odin's parallel serialisation, without distinguishing between them. Jiangyu's serialised type tree correctly reflects only the Unity-native layer, while the Odin layer lives inside the `serializationData` blob.
...
4. Check other template types for the Odin serialisation pattern (look for `serializationData` + interface/abstract-typed legacy fields)
