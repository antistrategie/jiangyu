...
- **ACP** (`Jiangyu.Acp`): standalone Agent Client Protocol library at `src/Jiangyu.Acp/`. Hand-rolled JSON-RPC 2.0 over Content-Length framed stdio (same convention as LSP). Zero Jiangyu-specific dependencies; could be extracted as a NuGet package. Schema types cover initialise, session, prompt, session updates (10 polymorphic subtypes), permissions, filesystem, and terminal callbacks. `AcpClient` is the public API; `IAcpClientHandler` receives agent-to-client callbacks. Tests at `tests/Jiangyu.Acp.Tests/`.
