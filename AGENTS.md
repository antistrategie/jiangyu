...
- `PREFAB_CLONING_TODO.md`: roadmap for the prefab-cloning feature area (template-prefab linkage, authoring UX with a scaffolded `unity/` project, runtime resolution). Read when working on prefab cloning, asset additions of type `GameObject`, or mod-side Unity authoring tooling.
...
- `unity/` directory in project root: per-mod Unity Editor project for prefab authoring, scaffolded by `jiangyu unity init`. Created on demand only when the mod needs Unity (data-only and replacement-only mods never pay the cost). Committed. The init command is idempotent: it refreshes Jiangyu-managed files under `unity/Assets/Jiangyu/` plus `.gitignore` each run, and preserves modder content (prefabs under `Assets/Prefabs/`, the `Packages/manifest.json` once seeded, anything under `ProjectSettings/`). See `PREFAB_CLONING_TODO.md`.
- Addition prefabs are GameObject assets shipped as AssetBundles satisfying KDL `asset="name"` references on `List<GameObject>`/`GameObject` template fields. Modder authoring lives at `unity/Assets/Prefabs/<name>.prefab`; `mise compile` invokes Unity batchmode (`-executeMethod Jiangyu.Mod.BuildBundles.BuildAll`) against `unity/` to build each prefab into a per-name bundle under `.jiangyu/unity-build/`. Pre-built bundles dropped under `assets/additions/prefabs/<name>.bundle` are the escape hatch (e.g. shared from another mod, built externally). Both sources feed `AdditionPrefabStaging.Stage`, which copies bundles into `compiled/` and records names on `manifest.additionPrefabs` for the loader.
