...
- `type="TypeName"` is the single construction keyword. There is no `handler=` or `composite=` keyword. `type=` always constructs a fresh instance, naming the concrete type; the compiler picks the storage mechanism from the destination field, and a plain (no-`type=`) descent edits in place:
  - **append / insert** (`append "Field" type="X" { ... }`): a `CompiledTemplateValueKind.TypeConstruction` value. `ValidateTypeConstruction` accepts collection elements and polymorphic-scalar destinations; `TryConstructPolymorphic` / `TryConstructComposite` instantiates the named type via reflection (ScriptableObject / Il2CppObjectBase / pure managed) and the wrapper-cast in `ApplyAndVerify` lets the assignment cross IL2CPP-stripped interface boundaries.
  - **set element** (`set "Field" index=N type="X" { ... }`): a `TypeConstruction` value carrying `Index`. `TrySetElement` constructs a fresh `X` (validated against the collection's declared element type) and overwrites slot N. Inner ops populate it, every other field defaults. A bogus subtype still fails.
  - **edit element** (`set "Field" index=N { ... }`, no `type=`): a `TemplateDescentStep` carrying just `Field` and `Index` (no subtype). `TryDescendElement` reads the live element's concrete type (`il2cpp_object_get_class`), casts via `Il2CppSubtypeResolver` / `Il2CppReflectiveCast`, and edits in place so sibling fields survive. The compiler validates each inner field against the union of the element base's concrete subtypes (`TemplateMemberQuery` `unionFallbackBase`); the live cast is authoritative.
  - **set value / polymorphic scalar** (`set "Field" type="X" { ... }`, no `index=`): a value-type composite or tagged-string destination is demoted to `CompiledTemplateValueKind.Composite` at compile (`TemplateCatalogValidator` routes by destination, the loader builds it inline or packs the tagged string); a polymorphic-scalar reference keeps the `TypeConstruction` path and is constructed and assigned across the interface boundary.
  - The preview applier (`TemplateInspectionPreviewApplier`) mirrors all of it: `BuildNode` renders `TypeConstruction`, and the descent path edits the element node in place. The loader's IL2CPP construct/cast path is a live-game smoke; the preview path is covered by `TemplateInspectionPreviewTests`.
