...
- **In-place `Texture2D` mutation is the validated runtime replacement strategy for Jiangyu.** The 2026-04-18 gating experiment (`InPlaceMutationExperiment`) showed that `Graphics.ConvertTexture` + `Graphics.CopyTexture` mutates the game texture in place and every consumer (materials, UGUI, UI Toolkit, template references, manager caches) inherits the change. The current `SkinnedMeshRenderer.sharedMaterials` consumer-walk via `MaterialReplacementService.GetOrCreateDirectTextureReplacementMaterials` is subtly incorrect — it allocates new `Material` instances that drop the game's `MaskMap`/`NormalMap`/`EffectMap` slots, so the shader runs without them. In-place mutation preserves the full shader pipeline. Sprite replacement piggybacks on texture replacement because every `Sprite` references a backing `Texture2D`; compile-time atlas detection must refuse sprite targets whose backing texture backs more than one indexed `Sprite`. See `TODO.md` ("Runtime Replacement Strategy — Validated, Ready To Implement") and `docs/research/investigations/2026-04-18-sprite-audio-runtime-routing.md` for the full evidence and implementation agenda. The current consumer-walk sweeps in `ReplacementCoordinator.cs` remain in place until the in-place path ships as a default.
- **Audio replacement is blocked on an Il2CppInterop marshalling bug.** `AudioClip.GetData(float[], int)` throws `Object was garbage collected in IL2CPP domain` on this MelonLoader 0.7.2 + Unity 6 combination regardless of clip origin. This is the same bug class as the ICall issues worked around in `BundleLoader.cs` — the fix pattern is a hand-written ICall wrapper that bypasses Il2CppInterop's broken `float[]` marshalling. Audio runtime replacement cannot be tested against MENACE consumers until that wrapper lands. Do not retire the scene-scoped `AudioSource.clip` sweep until then.
- Investigation tooling: `Jiangyu.Loader.Diagnostics.RuntimeInspector` writes a JSON dump of live sprite and audio identity (both scene-scoped and `Resources.FindObjectsOfTypeAll`-scoped) to `<UserData>/jiangyu-inspect/` on each scene load, when the file `<UserData>/jiangyu-inspect.flag` exists. Use this to answer "what does MENACE actually have in memory right now" questions before adding code. Extend the inspector — don't write one-off diagnostic scripts — as new identity surfaces become relevant.
