...
/// <summary>
/// Shared infrastructure for the loader's diagnostic inspectors. Owns the output
/// directory, per-kind retention sweep, and the JSON serialiser settings, and reads
/// its gating from the <see cref="DevFlags"/> <c>inspect</c> toggle. Both
...
///
/// Enabled by the <c>inspect</c> toggle in the <c>jiangyu-flags</c> file: bare
/// (<c>inspect</c>) keeps dumps forever, <c>inspect=N</c> keeps at most N files per
/// kind in <c>&lt;UserData&gt;/jiangyu-inspect/</c>, deleting the oldest after each write.
/// </summary>
internal static class InspectionSink
{
    ...
    internal static FlagState GetCachedFlag()
        => new(DevFlags.IsEnabled("inspect"),
            int.TryParse(DevFlags.Value("inspect"), out var cap) && cap > 0 ? cap : (int?)null);
