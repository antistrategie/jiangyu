...
/// <summary>
/// Installs a user-level <c>.desktop</c> entry on first launch so Wayland
/// compositors can resolve the app's taskbar/dock icon.
/// </summary>
/// <remarks>
...
/// Wayland we have to plant a desktop entry or every user falls back to the
/// compositor's default icon. VS Code, Obsidian, JetBrains all do this.
/// </remarks>
internal static class LinuxDesktopEntry
{
    ...
    /// <summary>
    /// Write the desktop entry if it's missing. Safe to call unconditionally
    /// on every launch — the file-exists check is the short-circuit.
    /// </summary>
    public static void Ensure()
    {
        try
        {
            ...
            File.WriteAllText(desktopPath, Render(iconPath, exe));
        }
        catch (Exception ex)
        {
    ...
    private static string Render(string iconPath, string exe) =>
        $"""
        [Desktop Entry]
        Type=Application
        Name={DisplayName}
