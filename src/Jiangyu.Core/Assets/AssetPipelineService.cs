...
public sealed class AssetPipelineService(string gameDataPath, string cachePath, IProgressSink progress, ILogSink log)
{
            ...
            foreach (IUnityObjectBase asset in collection)
            {
                ...
                else if (asset is AssetRipper.SourceGenerated.Classes.ClassID_114.IMonoBehaviour monoBehaviour)
                {
                    if (entry.Name is { } entryName && IsSoundBankAssetName(entryName))
                    {
                        ...
                        {
                            entry.NamedChildren = ExtractSoundBankSoundNames(inspection);
                            entry.BankId = ExtractSoundBankBankId(inspection);
                        }
                    }
                    else if (IsConversationTemplate(monoBehaviour))
                    {
                        ...
                        if (inspection is not null)
                        {
                            entry.Roles = ExtractConversationRoles(inspection);
                            entry.Path = ExtractConversationPath(inspection);
    ...
    /// already-inspected SoundBank, or null when there are no sounds. Used
    /// to populate <see cref="AssetEntry.NamedChildren"/>.</summary>
    ...
    /// unparseable. Used to populate <see cref="AssetEntry.BankId"/> so
    /// compile-time bank-name → bankId resolution doesn't rely on a
    /// hardcoded table that drifts with MENACE updates.</summary>
    ...
    /// </summary>
    private static bool IsConversationTemplate(
        AssetRipper.SourceGenerated.Classes.ClassID_114.IMonoBehaviour monoBehaviour)
    {
    ...
    /// speaker conversations). Used by the loader's path-keyed lookup
    /// and Studio's from= dropdown.
    /// </summary>
    ...
    /// Used to populate <see cref="AssetEntry.Roles"/> so compile-time
    /// <c>set "RoleGuid" "Entity"</c> resolution and Studio's role
    /// combobox both consume one source of truth.
    /// </summary>
