...
vi.mock("@shared/ui/Modal/Modal.module.css", () => ({
  default: new Proxy({}, { get: (_, key) => key }),
}));

vi.mock("@shared/ui/MenuList/MenuList.module.css", () => ({
  default: new Proxy({}, { get: (_, key) => key }),
}));

vi.mock("@features/agent/registry", async () => {
  const actual = await vi.importActual<typeof import("@features/agent/registry")>(
